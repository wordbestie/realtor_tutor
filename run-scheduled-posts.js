...
exports.handler = async () => {
  ...
  try {
    const { data, error } = await admin
      ...
      .select('id,caption,platforms,media_type,media_url,image_url,yt_title,yt_description,webhook')
      .eq('status', 'pending')
  ...
  for (const row of due) {
    ...
    // 4) deliver
    try {
      const url = row.media_url || row.image_url;
      const mediaType = row.media_type || 'photo';
      const youtube = (row.yt_title || row.yt_description)
        ? { title: row.yt_title || '', description: row.yt_description || '' } : {};
      const r = await fetch(row.webhook, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ mediaType, mediaUrl: url, imageUrl: url, caption: row.caption, platforms: row.platforms || {}, youtube })
      });
